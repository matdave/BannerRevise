...
class AdPositionUpdateProcessor extends modObjectProcessor
{
    ...
    function process()
    {
        if (!$position = $this->getProperty('position')) {return $this->modx->error->failure($this->modx->lexicon('bannerrevised.positions.error.ns'));
        }
        if (!$ad = $this->getProperty('ad')) {return $this->modx->error->failure($this->modx->lexicon('bannerrevised.ads.error.ns'));
        }
        ...
        if ($this->modx->getCount('brevAdPosition', $arr)) {return $this->modx->error->failure($this->modx->lexicon('bannerrevised.adposition.error.ae'));
        }
